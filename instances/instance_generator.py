...
def generate_instance(nMachines, jobRange, instanceType, instanceName):
	...
	'''with open(instanceName + '_%i_%i_%c.dzn' % (nMachines, nJobs, instanceType), "w") as mzn_file:
		mzn_file.write('nJobs = %i;' % nJobs)
		mzn_file.write('\n')
		mzn_file.write('nMachines = %i;' % nMachines)
		mzn_file.write('\n')

		mzn_file.write('capableMachines = [')
		mzn_file.write('\n')
		for j in range(0, nJobs):
			mzn_file.write('{' + ','.join(str(m + 1) for m in capableMachines[j]) + '},')
			mzn_file.write('\n')

		mzn_file.write('];')  
		mzn_file.write('\n')  

		mzn_file.write('durations = [|')
		mzn_file.write('\n')
		for j in range(0, nJobs):
			mzn_file.write(','.join([ str(durations[(j,m)]) for m in range(0,nMachines) ]) + '|')
			if j == nJobs - 1:
				mzn_file.write('];')
			else:
				mzn_file.write('\n')
		...
		mzn_file.write('releaseDates = [|')
		mzn_file.write('\n')
		for j in range(0, nJobs):
			mzn_file.write(','.join([ str(releaseDates[(j,m)]) for m in range(0,nMachines) ]) + '|')
			if j == nJobs - 1:
				mzn_file.write('];')
			else:
				mzn_file.write('\n')
		...
		mzn_file.write('setupTimes = array3d(1..nJobs,1..nJobs,1..nMachines,[')
		mzn_file.write('\n')
		for j in range(0, nJobs):
			for k in range(0, nJobs):
				mzn_file.write(','.join([ str(setupTimes[(j,k,m)]) for m in range(0,nMachines) ]) + ',\t')
		...
		mzn_file.write('horizon = %i;' % horizon)'''
